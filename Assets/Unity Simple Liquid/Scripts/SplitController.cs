...
namespace UnitySimpleLiquid
{
    ...
    public class SplitController : MonoBehaviour
    {
		...
		private bool hasAcidChanged = false;
		private bool hasBaseChanged = false;

		private bool hasPouredAcid = false;
		private bool hasPouredBase = false;
        ...
        public void StartEffect(Vector3 splitPos, float scale)
        {
		...
		private void TransferLiquid(RaycastHit hit, float lostPercentAmount, float scale)
        {
			...
			if (insideRadius)
			{
				...
				SendLiquidContainer(liquid.liquidContainer, colliderTag);
			}
		...
		private void SendLiquidContainer(LiquidContainer lc, string lcCollider)
		{
			...
			//if Acid / Base is mixed with Indicator
			if(gameObject.tag == "Indicator"){
				if(!hasFiredEvent) {
					hasFiredEvent = true;
					FindObjectOfType<AudioManager>().PlayAudio(8);
					}
				if(lcCollider == "Acid"){
					if(lc.LiquidColor.g <= .7f && lc.LiquidColor.b <= .7f && !hasAcidChanged){
						hasAcidChanged = true;
						GameEvents.instance.ColorChange(hasAcidChanged, hasBaseChanged);
					}
					newColor = new Color(1, 0, 0, lc.LiquidColor.a);
				}
				else if(lcCollider == "Base"){
					if(lc.LiquidColor.g <= .7f && !hasBaseChanged){
						hasBaseChanged = true;
						GameEvents.instance.ColorChange(hasAcidChanged, hasBaseChanged);
					}
					newColor = new Color(.8f, 0, 1, lc.LiquidColor.a);
				}
			}

			// if Acid / Base is mixed with Flask
			if(lcCollider == "Flask"){
				if(gameObject.tag == "Acid" && !hasPouredAcid){
					hasPouredAcid = true;
					GameEvents.instance.AcidBasePour(hasPouredAcid, hasPouredAcid);
				}

				if(gameObject.tag == "Base" && !hasPouredBase){
					hasPouredBase = true;
					GameEvents.instance.AcidBasePour(hasPouredAcid, hasPouredBase);
				}
			}


			//we find the coefficient of the volume of the tank and the volume of the incoming fluid
			float volume = lc.FillAmount;
